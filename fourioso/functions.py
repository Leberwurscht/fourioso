...
def transform(axis, data=None, order=1, phase_coeff=None, return_phase_coeff=False, overwrite=False, return_axis=True
  ...
  if return_phase_coeff:
    if data is not None:
      if return_axis: return axis_out, data_out, additional_linear_phase
      else: return data_out, additional_linear_phase
    else:
      if return_axis: return axis_out, additional_linear_phase
      else: return additional_linear_phase
  else:
    if data is not None:
      if return_axis: return axis_out, data_out
